...
function getInputs() {
		...
		//get gains
		gains = Number(document.getElementById("gains").value);
...
function calculateEntropyValue(passwordLength, gains, losses) {
		//use passwordLength to calculate entropy bits
		var hValue = 0;
		if (passwordLength === 2) {
				hValue = passwordLength + 3;
			}
		else if (passwordLength === 2) {
				hValue = passwordLength + 4;
			}
		else {
				hValue = passwordLength + 5;
			}
		hValue = hValue + gains + losses;
		
		calcDisplayText = "LOA1 Calculation: " + "<br/>"+ "Entropy bits ( Adjusted Password Length + Gains In Entropy - Losses In Entropy): " + hValue + " "; 
		
		calcDisplayText2 = "LOA2 Calculation: " + "<br/>"+ "Entropy bits ( Adjusted Password Length + Gains In Entropy - Losses In Entropy): " + hValue + " "; 
...
function calculateAll(passwordLength, gains, losses, lifetimeInDays, lockoutInHours, guesses) {
		...
		var hValue = calculateEntropyValue(passwordLength, gains, losses);
...
function dataValidation() {
		...
		if ( isNaN(passwordLength) || passwordLength < 2 || passwordLength > 15 ) {
			dataValidated = false;
		...
		//checking gains
		if ( isNaN(gains) || gains < 0 || gains > 5 ) {
			dataValidated = false;
			console.log("gains");
			}
			
		//checking losses
		if ( isNaN(losses) || losses < -3 || losses > 0 ) {
			dataValidated = false;
			console.log("losses");
			}			
...
document.getElementById("calculateButton").onclick = function() {
		...
		if (continueCalc === false) {
			...
			calculateAll(passwordLength, gains, losses, lifetimeInDays, lockoutInHours, guesses);
