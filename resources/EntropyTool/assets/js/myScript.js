...
function calculateEntropyValue(passwordLength, gains, losses) {
		var hValue = 0;
		hValue = (passwordLength + 1)*2 + Math.max(passwordLength-8 , 0)*1.5 + gains + losses;
		
		calcDisplayText = "LOA1 Calculation: " + "<br/>"+ "Entropy bits ( (Password Length + 1)*2 + Max(Password Length -8, 0)*1.5 + Gains In Entropy - Losses In Entropy): " + hValue + " "; 
		
		calcDisplayText2 = "LOA2 Calculation: " + "<br/>"+ "Entropy bits ( (Password Length + 1)*2 + Max(Password Length -8, 0)*1.5 + Gains In Entropy - Losses In Entropy): " + hValue + " "; 
...
function dataValidation() {
		...
		//checking gains
		if ( isNaN(gains) || gains < 0 || gains > 12 ) {
			dataValidated = false;
			console.log("gains");
			}
			
		//checking losses
		if ( isNaN(losses) || losses < -4 || losses > 0 ) {
			dataValidated = false;
			console.log("losses");
			}			
...
/*
document.getElementById("showCalculation").onclick = function() {
	document.getElementById("calcDisplay").innerHTML = calcDisplayText;	
	document.getElementById("calcDisplay2").innerHTML = calcDisplayText2;	
